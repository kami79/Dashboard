...
def ericsson_dash():
    ...
    if f1 is not None:
        ...
        os.chdir(r"C:\Users\Kamiran.Gabriel\desktop")
        sheets = read_excel_sheets("NEW_EXCELL_Dash_V1.0.xlsx")
        st.write(list(sheets.values())[0].head())
    
    display_dashboard(sheets, "Ericsson")

# Function to handle Huawei Dash
def huawei_dash():
    st.title(":chart_with_upwards_trend: Huawei Dash")
    st.markdown("""<style>div.block-container {padding: 1rem;}</style>""", unsafe_allow_html=True)
