...
def read_excel_sheets(file):
    ...
    for sheet_name in xls.sheet_names:
        ...
        elem_column = detect_elem_column(df)
        sheets[sheet_name] = df.rename(columns={elem_column: 'ELEM'})
    return sheets

def detect_elem_column(df):
    elem_columns = ['ELEM', 'element', 'SN']  # Add other possible column names
...
def huawei_dash():
    ...
    f2 = st.sidebar.file_uploader(":file_folder: Upload a file", type=["csv", "txt", "xlsx", "xls"], key="Huawei Dash_file", help="Upload a file here")
    if f2 is not None:
        filename = f2.name
        st.write(filename)
        if filename.endswith('.csv'):
...
def show_kpi_summary(sheets, dash_type):
    ...
    if available_metrics:
        ...
        elem_filters = st.sidebar.multiselect("Select ELEM", df["ELEM"].unique(), key=f"{dash_type}_elem_filters")
        ...
        if metrics_to_analyze:
            ...
            for metric in metrics_to_analyze:
                ...
                for elem in elem_filters:
                    filtered_df = df[df["ELEM"].isin([elem])]
            ...
            if len(elem_filters) > 1:
                fig = px.bar(summary_df.melt(id_vars="Metric"), x="Metric", y="value", color="variable", barmode="group", title="Summary Comparison", width=800, height=600)
                fig.update_layout(legend_title_text=None)
                st.plotly_chart(fig, use_container_width=True)
            else:
                st.warning("Select more than one ELEM to compare in the summary.")
        else:
            st.warning("Select metrics to analyze from the list.")
    else:
        st.error(f"No suitable metrics found in sheet '{selected_sheet}'.")
